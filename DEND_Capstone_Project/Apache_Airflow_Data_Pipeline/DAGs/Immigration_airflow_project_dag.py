...
dag = DAG('Immigration_Airflow_Project_Dag',
           ...
           start_date=datetime.datetime(2022, 6, 19, 0, 0, 0, 0)
    )
...
Stage_Dated_Arrival_Departure_to_redshift = StageToRedshiftOperator(
    task_id="Staging_Individual_Immigrants_Detail_Date_Records_table",
    dag=dag,
    redshift_conn_id="redshift",
...
#================================= LOADING TABLES FROM ( STAGING TO ====== lOADING TABLES) =========
...
Load_dim_Individual_Immigrants_Records_table = LoadDimensionOperator(
    task_id="Loading_dim_Individual_Immigrants_Records_table",
    dag=dag,
...
Load_dim_Dated_Arrival_Departure_table = LoadDimensionOperator(
    task_id="Loading_dim_Individual_Immigrants_Date_Records_table",
    dag=dag,
...
Load_dim_US_Citys_Temp_table = LoadDimensionOperator(
    task_id = "Loading_dim_US_City_Temperature_table",
    dag = dag,    
    redshift_conn_id = "redshift",
...
Load_dim_US_Citys_Demog_Race_table = LoadDimensionOperator(
    task_id = "Loading_dim_US_City_Demography_table",
    dag = dag,   
    redshift_conn_id = "redshift",
...
Load_dim_Port_Locations_table = LoadDimensionOperator(
    task_id = "Loading_dim_Port_Locations_table",
    dag = dag,    
    redshift_conn_id = "redshift",
...
Load_dim_Country_Codes_table = LoadDimensionOperator(
    task_id = "Loading_dim_Country_Codes_table",
    dag = dag,   
    redshift_conn_id = "redshift",
...
Load_dim_State_Codes_table = LoadDimensionOperator(
    task_id = "Loading_dim_State_Codes_table",
    dag = dag,    
    redshift_conn_id = "redshift",
    ...
    #data_quality_checks = [
       ...
       # {'check_sql': 'SELECT COUNT(*) FROM dim_Individual_Immigrants_Records \
        ...
        #{'check_sql': 'SELECT COUNT(*) FROM dim_US_City_Temperature \
          #                              WHERE City_Port is null', 'expected_result': 0},        
       # {'check_sql': 'SELECT COUNT(*) FROM dim_US_City_Demog_Race \
         #                               WHERE Demog_Port is null', 'expected_result': 0},
...
start_operator >> Clearing_Staging_Tables_Task    # clearing tables for staging and loading

# clearning tables before loading to staging tables
...
# Finishing staging tables
Stage_Fact_Immigration_to_redshift >> done_staging
Stage_Individual_Immigrants_Records_to_redshift >> done_staging
Stage_Dated_Arrival_Departure_to_redshift >> done_staging


Stage_US_Citys_Temp_to_redshift  >> done_staging             
Stage_US_Citys_Demog_Race_table_to_redshift >> done_staging
          
Stage_Port_Locations_to_redshift >> done_staging
Stage_Country_Codes_to_redshift >> done_staging
Stage_State_Codes_to_redshift  >> done_staging


# starting loading data from Staging tables to Loading tables
done_staging  >>  Load_Fact_Immigration_table
done_staging  >>  Load_dim_Individual_Immigrants_Records_table
done_staging  >>  Load_dim_Dated_Arrival_Departure_table
          
done_staging  >>  Load_dim_US_Citys_Temp_table
done_staging  >>  Load_dim_US_Citys_Demog_Race_table

done_staging  >>  Load_dim_Port_Locations_table
done_staging  >>  Load_dim_Country_Codes_table
done_staging  >>  Load_dim_State_Codes_table


# running data quality checks on all fact and dimension tables
...
#start_operator >> Clearing_Staging_Tables_Task   >> [ Stage_Fact_Immigration_to_redshift,  Stage_Individual_Immigrants_Records_to_redshift,
# Stage_Dated_Arrival_Departure_to_redshift,  Stage_US_Citys_Temp_to_redshift,  Stage_US_Citys_Demog_Race_table_to_redshift,                           #Stage_Port_Locations_to_redshift,  Stage_Country_Codes_to_redshift,  Stage_State_Codes_to_redshift ] >> done_staging >> [  #Load_Fact_Immigration_table, Load_dim_Individual_Immigrants_Records_table, Load_dim_Dated_Arrival_Departure_table, Load_dim_US_Citys_Temp_table,
# Load_dim_US_Citys_Demog_Race_table, Load_dim_Port_Locations_table, Load_dim_Country_Codes_table, Load_dim_State_Codes_table] >> #Run_Data_Quality_Checks >> end_operator
